...
- A fresh read of the public `go-shoot/x` repository tree confirms the published X DB is limited to the reviewed part/product JSON files and contains no separate mold/batch dataset; the current update index still exposes only the same `DrSw2` `+3g` weight note. The concrete Mold Batch checkbox therefore remains intentionally open.
