...
- Re-read the supplied anti-slop law while reviewing the changed surfaces. The implementation keeps content visible by default, uses the warm iron/clay palette, avoids neon/purple treatments and hover boops, preserves product image aspect ratios, and keeps the Assessment／Official／Stock data families visually separate.
- Current automated evidence: 61 test files / 288 tests passed, lint passed, typecheck passed, production build passed, and local smoke requests returned 200 for homepage, Part detail with query pagination, Events and Meta. The deployed site also returned 200 for its public root.
